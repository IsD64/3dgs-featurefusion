...
# Dynamically create FeaturedCamera as a proper NamedTuple
# with all Camera fields + feature_map
FeaturedCamera = NamedTuple('FeaturedCamera', [
    (name, Camera.__annotations__[name]) for name in Camera._fields
] + [('feature_map', torch.Tensor)])
FeaturedCamera.__new__.__defaults__ = (
    *Camera._field_defaults.values(),
    None,  # feature_map default
)


class FeaturedCameraDataset(CameraDataset):
    ...
    def to(self, device) -> 'FeaturedCameraDataset':
    ...
    def __getitem__(self, idx) -> FeaturedCamera:
        ...
        return FeaturedCamera(*camera, feature_map=feature_map)
